...
export function ContextSwitcher() {
  ...
  const { mode, activeOrganizationId, switchToPersonal, switchToOrganization } = useAccountStore();
    ...
    async function loadOrgs() {
      ...
      for (const m of memberships) {
        ...
        if (org && org.status === 'active') {
          loadedOrgs.push({ id: org.id, name: org.name, role: m.role });
        }
      }
      setOrgs(loadedOrgs);
      ...
      if (user.accountType === 'managed') {
        ...
        const personalVaults = await db.vaults.filter((v: any) => v.ownershipType === 'personal' && (v.ownerUserId === user.id || v.createdBy === user.id)).count();
        setHasPersonal(personalVaults > 0);
      }
    }
    loadOrgs();
  }, [user]);
