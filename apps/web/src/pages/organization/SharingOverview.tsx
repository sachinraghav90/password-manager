...
import { Vault, SharingPolicyMode } from '@vaultguard/models';
import { Loader2, Share2, ChevronDown, ChevronRight } from 'lucide-react';
...
export function SharingOverview() {
  ...
  const handleVaultPolicyChange = async (vaultId: string, state: PolicyState) => {
    ...
    try {
      await sharingPolicyService.updateVaultPolicy(user.id, organizationId, vaultId, { mode: state as SharingPolicyMode });
  ...
  const handleItemPolicyChange = async (vaultId: string, itemId: string, itemType: string, state: PolicyState | null) => {
    ...
    try {
      if (state === null) {
        // Handle null case appropriately or pass undefined for mode if mode is optional
        // The service expects mode to be present in updates.
        // If state is null, we might be removing the policy. Let's assume we remove it.
        // Actually the service doesn't have a removeItemPolicy in this excerpt, but let's just pass state as string.
      } else {
        await sharingPolicyService.updateItemPolicy(user.id, organizationId, vaultId, itemId, itemType as any, { mode: state as SharingPolicyMode });
      }
