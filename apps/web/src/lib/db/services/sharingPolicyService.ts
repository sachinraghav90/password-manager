...
import { OrganizationSharingPolicy, VaultSharingPolicy, ItemSharingPolicy, SharePermission, SharingPolicyMode, ItemType } from '@vaultguard/models';
...
export const sharingPolicyService = {
  ...
  async updateOrganizationPolicy(actorUserId: string, organizationId: string, updates: Partial<Omit<OrganizationSharingPolicy, 'id' | 'organizationId' | 'createdAt'>>): Promise<OrganizationSharingPolicy> {
    ...
    return await db.transaction('rw', db.organization_sharing_policies, db.audit_events, async () => {
      ...
      await auditService.logEvent({ organizationId, actorUserId, eventType: 'organization_sharing_policy_updated', metadata: { type: 'organization_sharing_policy', policyId: policy.id } });
      return policy;
    });
  },
  ...
  async updateVaultPolicy(
    ...
    return await db.transaction('rw', db.vault_sharing_policies, db.audit_events, async () => {
      ...
      await auditService.logEvent({ organizationId, actorUserId, eventType: 'vault_sharing_policy_updated', metadata: { type: 'vault_sharing_policy', policyId: policy.id, vaultId } });
      return policy;
    });
  },
  ...
  async updateItemPolicy(
    ...
    return await db.transaction('rw', db.item_sharing_policies, db.audit_events, async () => {
      ...
      await auditService.logEvent({ organizationId, actorUserId, eventType: 'item_sharing_policy_updated', metadata: { type: 'item_sharing_policy', policyId: policy.id, itemId, vaultId } });
      return policy;
    });
  },
