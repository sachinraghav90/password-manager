...
export function AppLayout() {
  ...
  const { user, logout, isSuperAdmin } = useAuthAdapter();
  const { theme, setTheme } = useSettingsAdapter();
  const { vaults, loadVaults } = useVaultAdapter();
  const { mode, activeOrganizationId, membershipRole } = useAccountAdapter();
