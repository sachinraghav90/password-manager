...
export function DashboardView() {
  ...
  const { mode, activeOrganizationId } = useAccountAdapter();
  const { vaults } = useVaultAdapter();
  const { entries, isLoading, loadItems, getItemDetails } = useItemListAdapter();
  ...
  useEffect(() => {
    if (user) {
      loadItems(user.id, mode === 'organization' ? activeOrganizationId : null);
    }
  }, [user?.id, loadItems, mode, activeOrganizationId]);
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        for (const entry of loginEntries) {
          try {
            const details = await getItemDetails(user.id, entry.vaultId, entry.id, 'login');
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="secondary" onClick={async () => {
            const handleReply = (event: MessageEvent) => {
              if (event.data?.type === 'VG_DEV_SYNC_REPLY') {
                window.removeEventListener('message', handleReply);
                if (event.data.success) {
                  alert('Sync Successful! Extension background replied: ' + JSON.stringify(event.data.res));
                } else {
                  alert('Sync Failed! Error from extension: ' + event.data.error);
                }
              }
            };
            window.addEventListener('message', handleReply);
            try {
              const vault = await import('@vaultguard/db-local').then(m => m.db.vaults.where('ownerUserId').equals(user!.id).first());
              window.postMessage({ type: 'VG_DEV_SYNC', user, vault }, '*');
              setTimeout(() => {
                window.removeEventListener('message', handleReply);
                alert('Sync Timeout! The extension content script did not intercept the message. Please ensure the extension is reloaded and YOU HARD REFRESHED (Ctrl+Shift+R) this web page.');
              }, 2000);
            } catch (e: any) {
              alert('Error gathering data for sync: ' + e.message);
            }
          }}>
            Debug: Sync to Extension
          </Button>
        </div>
      </div>
