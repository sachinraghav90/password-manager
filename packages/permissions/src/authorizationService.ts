...
import { OrganizationPermission } from '@vaultguard/models';
...
class AuthorizationService {
  ...
  /**
   * Evaluates permission and throws AuthorizationError if denied.
   */
  async assertHasPermission(
    actorUserId: string,
    organizationId: string,
    permission: OrganizationPermission,
    resourceType: AuthorizationResourceType = 'organization',
    resourceId?: string
  ): Promise<void> {
    const decision = await this.canPerform(actorUserId, permission, {
      organizationId,
      resourceType,
      resourceId,
    });
    ...
    if (!decision.allowed) {
      throw new AuthorizationError(`Permission denied: ${permission}`, decision.reason);
    }
  }

  async hasPermission(actorUserId: string, organizationId: string, permission: OrganizationPermission): Promise<boolean> {
    const decision = await this.canPerform(actorUserId, permission, {
