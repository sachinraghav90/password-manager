...
import { ItemType, BaseItemRecord, PMItemIndex } from '@vaultguard/models';
...
export abstract class ItemTypeAdapter<TForm, TRecord extends BaseItemRecord, TDecrypted> {
  ...
  async create(form: TForm, context: ItemOperationContext): Promise<TDecrypted> {
    ...
    const title = (form as any).name || (form as any).title || 'Untitled';
...
import { LoginFormState, LoginPayload, normalizeLoginWebsites } from '@vaultguard/models';
import { PMLogin } from '@vaultguard/models';
...
export class LoginAdapter extends ItemTypeAdapter<LoginFormState, PMLogin, PMLogin & { payload: LoginPayload }> {
  ...
  protected payloadFromForm(form: LoginFormState): LoginPayload {
    ...
    // Persist one canonical website representation for every newly created/updated login.
    payload.websites = normalizeLoginWebsites(payload as Record<string, any>);
    return payload;
  }
