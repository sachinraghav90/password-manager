...
import { ShareRequest, SharePermission, ShareTargetType, ShareResourceType, ItemType } from '../schema';
...
export const shareRequestService = {
  ...
  async createShareRequest(
    ...
    await db.transaction('rw', db.share_requests, db.audit_events, async () => {
      ...
      await auditService.logEvent(organizationId, actorUserId, 'share_request_created', 'share_request', request.id);
    });
  ...
  async approveShareRequest(actorUserId: string, organizationId: string, requestId: string): Promise<void> {
    ...
    await db.transaction('rw', db.share_requests, db.share_grants, db.audit_events, async () => {
      ...
      if (!effectivePolicy.allowSharing) {
        ...
        await auditService.logEvent(organizationId, actorUserId, 'share_request_rejected', 'share_request', req.id, { reason: 'Policy changed making request invalid' });
      ...
      await auditService.logEvent(organizationId, actorUserId, 'share_request_approved', 'share_request', req.id);
    });
  },
  ...
  async rejectShareRequest(actorUserId: string, organizationId: string, requestId: string): Promise<void> {
    ...
    await db.transaction('rw', db.share_requests, db.audit_events, async () => {
      ...
      await auditService.logEvent(organizationId, actorUserId, 'share_request_rejected', 'share_request', req.id);
    });
  }
