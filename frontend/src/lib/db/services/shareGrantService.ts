...
import { ShareGrant, SharePermission, ShareResourceType, ItemType } from '../schema';
...
export const shareGrantService = {
  ...
  async createShareGrant(
    ...
    await db.transaction('rw', db.share_grants, db.audit_events, async () => {
      ...
      await auditService.logEvent(organizationId, createdByUserId, 'share_grant_created_pending_crypto', 'share_grant', grant.id);
    });
  ...
  async revokeGrant(actorUserId: string, organizationId: string, grantId: string): Promise<void> {
    ...
    await db.transaction('rw', db.share_grants, db.audit_events, async () => {
      ...
      await auditService.logEvent(organizationId, actorUserId, 'share_grant_revocation_requested', 'share_grant', grant.id);
    });
  }
