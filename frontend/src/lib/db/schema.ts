...
export interface VaultItem {
  ...
  type: 'Login' | 'SecureNote' | 'CreditCard' | 'Identity' | 'Secret';
  title: string; // Plaintext, for search
  websiteUrl: string; // Plaintext, for search/favicon
  favorite: boolean; // Plaintext, for easy filtering
  encryptedData: string; // Base64 ciphertext of VaultItemPayload JSON
  dataNonce: string; // Base64 IV used for encryption
  encryptionVersion: string; // e.g. 'AES256GCM'
  createdAt: number;
  updatedAt: number;
}
