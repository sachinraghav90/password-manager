...
import { sharingPolicyService, EffectivePolicy } from '../../lib/db/services/sharingPolicyService';
    ...
    async function init() {
      ...
      try {
        ...
        const effPolicy = itemId 
          ? await sharingPolicyService.getEffectiveItemPolicy(organizationId, vaultId, itemId)
          : await sharingPolicyService.getEffectiveVaultPolicy(organizationId, vaultId);
        
        setPolicy(effPolicy);

        // 2. Load User's Teams
        if (effPolicy.allowSharing && effPolicy.allowTeamTargets) {
          const userMembership = await db.organization_memberships.where('[organizationId+userId]').equals([organizationId, user!.id]).first();
          if (userMembership) {
            ...
            for (const tId of teamIds) {
              const t = await db.organization_teams.get(tId);
              if (t) teams.push(t);
            }
            setUserTeams(teams);
          }
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      ...
      try {
        ...
        const memberships = await db.organization_memberships.where('organizationId').equals(organizationId).toArray();
        ...
        for (const uid of userIds) {
          ...
          if (u && (u.email.toLowerCase().includes(query) || u.fullName.toLowerCase().includes(query))) {
            results.push(u);
          }
        }
